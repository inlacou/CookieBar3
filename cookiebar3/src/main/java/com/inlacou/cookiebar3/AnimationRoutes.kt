...
import com.inlacou.cookiebar3.interpolators.easetypes.EaseInBounce
import com.inlacou.cookiebar3.interpolators.easetypes.EaseInOutBounce
import com.inlacou.cookiebar3.interpolators.easetypes.EaseOutBounce
...
object AnimationRoutes {
	...
	val showFromBottom by lazy { listOf(
			...
			CookieAnimationStep(tag = "appear and anchor", duration = DEFAULT_TIME_MOVEMENT_ANIMATION, interpolator = EaseInOutBounce(), holdOnPosition = DEFAULT_TIME_ABOVE_BOTTOM_BAR, animation = CookieAnimation({
	...
	val showFromBottom2 by lazy { listOf(
			CookieAnimationStep(tag = "basic init", duration = 0, animation = CookieAnimation({
				//Basic initialization
				centerInParent(horizontal = true, vertical = false)
				invisible()
	...
	val showFromTop by lazy { listOf(
			...
			CookieAnimationStep(tag = "appear and anchor", duration = DEFAULT_TIME_MOVEMENT_ANIMATION, interpolator = EaseOutBounce(), holdOnPosition = DEFAULT_TIME_ABOVE_BOTTOM_BAR, animation = CookieAnimation({
				//Appear and anchor bottom
	...
	val specialExample by lazy { listOf(
			...
			CookieAnimationStep(tag = "appear, center and anchor", duration = DEFAULT_TIME_MOVEMENT_ANIMATION, holdOnPosition = DEFAULT_TIME_CENTER_OF_SCREEN, animation = CookieAnimation({
				//Appear and move to center
				centerInParent(horizontal = false, vertical = true)
			})),
			CookieAnimationStep(tag = "botton and resize, and anchor", duration = DEFAULT_TIME_MOVEMENT_ANIMATION, holdOnPosition = DEFAULT_TIME_ABOVE_BOTTOM_BAR, animation = CookieAnimation({
